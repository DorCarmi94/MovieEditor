...
namespace glfw
{
...

  {
    ...
    for (int i = 0; i < sizeof(texIDs) / sizeof(texIDs[0]); i++) {
        texIDs[i] = i;
        slots[i] = i;
    }
    layers = 0;
...
IGL_INLINE bool Viewer::load_mesh_from_data(const Eigen::MatrixXd &V,
  ...
  IGL_INLINE void Viewer::open_dialog_load_mesh()
  {
      ...
      ShapeTransformation(scaleAll, 2.0f, 0);
      ShapeTransformation(xTranslate, 0, 0);
      ShapeTransformation(yTranslate, 0, 0);
      ShapeTransformation(zTranslate, 0, 0);
    /*std::string fname = igl::file_dialog_open();
  ...
  // Material
  IGL_INLINE void Viewer::open_dialog_load_material()
  {
      std::string fname = igl::file_dialog_open();
      ...
      this->load_material_from_file(fname.c_str());
  }


  IGL_INLINE bool Viewer::load_material_from_file(
      const std::string& material_file_name_string)
  {
      AddTexture(material_file_name_string, 2);
      // todo maybe use this for verifacation later

     /* size_t last_dot = material_file_name_string.rfind('.');
      if (last_dot == std::string::npos)
      {
          std::cerr << "Error: No file extension found in " <<
              material_file_name_string << std::endl;
          return false;
      }

      std::string extension = material_file_name_string.substr(last_dot + 1);

      if (extension == "off" || extension == "OFF")
      {
          Eigen::MatrixXd V;
          Eigen::MatrixXi F;
          if (!igl::readOFF(material_file_name_string, V, F))
              return false;
          data()->set_mesh(V, F);
      }*/
  ...
  IGL_INLINE void Viewer::AddLayer()
  {
      layers++;
  }
...
// Axis, Plane, Cube, Octahedron, Tethrahedron, LineCopy, MeshCopy
    ...
    void Viewer::ClearPickedShapes(int viewportIndx)
    {
        ...
        if (selected_data_index > 0) {
            SetShapeShader(selected_data_index, 0);
        }
        selected_data_index = 0;
    ...
    void Viewer::MouseProccessing(int button, int xrel, int yrel, float movCoeff, Eigen::Matrix4d cameraMat,int viewportIndx)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                std::cout << "End mouse pressing" << std::endl;
            }
        }
    }
    ...
    int Viewer::AddTextureForCubeMap(const std::vector<std::string>& textureFileNames)
    {
        texture_index++;
        textures.push_back(new Texture(textureFileNames));
        return(textures.size() - 1);
    }
